...
class Runner(submitit.helpers.Checkpointable):
    ...
    def __call__(self, config) -> None:
        with new_trainer_context(args=args, config=config) as ctx: # context manager
            self.config = ctx.config
            self.task = ctx.task
            self.trainer = ctx.trainer

            self.task.setup(self.trainer) # prepare trainer, e.g., load checkpoints if ckpt is not None
            self.task.run() # mode "train": self.trainer.train(). mode 'predict': self.trainer.predict()
...
if __name__ == "__main__":
    ...
    config = build_config(args, override_args) # config is dict
