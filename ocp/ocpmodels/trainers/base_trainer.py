...
class BaseTrainer(ABC):
    def __init__(
        ...
        if distutils.is_master():
            ...
            run_cfg_path = os.path.join(run_dir, 'run_setting.yml')
            with open(run_cfg_path, 'w') as file:
    ...
    def load_datasets(self) -> None:
        ...
        if self.config["dataset"].get("src", None):
            ...
            # lmdb: ocpmodels/datasets/lmdb_dataset
            ...
            if self.config.get("val_dataset", None):
                if self.config["val_dataset"].get("use_train_settings", True):
                    ...
                    val_config.update(self.config["val_dataset"]) # src
                else:
    ...
    def load_task(self):
        ...
        ) # eval_metrics: {name1: metric1, name2: metric2, ...}, e.g. {'y': 'mae'}
    ...
    def load_checkpoint(
        ...
        self.early_stop = checkpoint.get("early_stop", None)
        self.initial_source_loss = checkpoint.get("initial_source_loss", None)
        ...
        if "normalizers" in checkpoint and checkpoint["normalizers"] is not None:
            for key in checkpoint["normalizers"]:
                ...
                if target_key in self.normalizers:
                    ...
                    logging.info(f'load normalize: {target_key}')

        if self.scaler and "amp" in checkpoint and checkpoint["amp"]:
    ...
    def load_optimizer(self) -> None:
        ...
        if weight_decay > 0:
            ...
            if self.config["linear_probe"]:
                updated_params = []
                for name, param in self.model.named_parameters():
                    if 'prop_head' in name:
                        logging.info(f'Using linear probe! Update {name}')
                        updated_params.append(param)
                    else:
                        param.requires_grad = False
            else:
                updated_params = self.model.parameters()
                logging.info('Update all params!')
                ...
                return save_checkpoint(
                    {
                        ...
                        "initial_source_loss": self.initial_source_loss if hasattr(self, 'initial_source_loss') else None,
                    },
    ...
    def update_best(
        ...
        if (
            ...
            self.early_stop -= 1
            logging.info(f'Not improved, patience decrease to {self.early_stop}')
    ...
    def validate(self, split: str = "val", disable_tqdm: bool = False):
        ...
        for i, batch in tqdm(
            ...
            # add {'loss': {'total': no., 'numel': no., 'metric': no.}}
