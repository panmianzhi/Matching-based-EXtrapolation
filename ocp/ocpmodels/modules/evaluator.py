    ...
    def eval(
        ...
        for target_property in self.target_metrics:
            for fn in self.target_metrics[target_property]:
                ...
                res = eval(fn)(prediction, target, target_property) # eval(fn) are funcs defined following
...
# add by panmz 24/09/11
def spearman(
    prediction: Dict[str, torch.Tensor],
    target: Dict[str, torch.Tensor],
    key=slice(None),
) -> Dict[str, Union[float, int]]:
    spear, _ = spearmanr(prediction[key].view(-1).cpu().numpy(), 
                         target[key].view(-1).cpu().numpy())
    return {
        "metric": spear,
        "total": spear,
        "numel": 1,
...
def pearson(
    ...
    pear, _ = pearsonr(prediction[key].view(-1).cpu().numpy(), 
                         target[key].view(-1).cpu().numpy())
    return {
        "metric": pear,
        "total": pear,
...
def NLLLoss(
    ...
    error = torch.nn.functional.nll_loss(prediction[key], target[key].view(-1))
    return {
        "metric": torch.mean(error).item(),
        "total": torch.sum(error).item(),
        "numel": error.numel(),
...
def accuracy(
    ...
    assert prediction[key].dim() == 2, f'prediction:{prediction[key].shape}'
    predicted_labels = torch.argmax(prediction[key], dim=1)
    correct_predictions = (predicted_labels.view(-1) == target[key].view(-1)).float()
    return {
        "metric": torch.mean(correct_predictions).item(),
        "total": torch.sum(correct_predictions).item(),
        "numel": correct_predictions.numel(),
    }