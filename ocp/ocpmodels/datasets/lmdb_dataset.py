...
class LmdbDataset(Dataset[T_co]):
    ...
    def __init__(self, config) -> None:
        ...
        if not self.path.is_file():# this
            ...
            keylens = [len(k) for k in self._keys] # [len_lmdb1, len_lmdb2, ..., len_lmdbN]
    ...
    def compute_sample_weights(self) -> List:
        ...
        for idx in range(self.num_samples):
            ...
            if not self.path.is_file():
                ...
                properties.append(pickle.loads(datapoint_pickled).y)
            else:
        ...
        assert len(sample_weights) == self.num_samples, \
            f'num_samples: {self.num_samples} v.s. ampled_weights len: {len(self.sample_weights)}'
    ...
    def __getitem__(self, idx: int) -> T_co:
        ...
        if not self.path.is_file():
            ...
            data_object.fid = data_object.fid = torch.LongTensor([el_idx])
        else:
