...
# panmz: this implement will NOT preserve gradient from other GPUS
# so DONOT use this for bigger size contrastive learning!
def all_gather(
...
class AllGather(torch.autograd.Function):
    ...
    def forward(ctx, tensor):
        # Get the size of the tensor on each rank
        local_size = torch.tensor([tensor.shape[0]], device=tensor.device)
        all_sizes = [torch.zeros_like(local_size) for _ in range(get_world_size())]
        dist.all_gather(all_sizes, local_size)
        max_size = max(all_sizes)
        
        # Pad tensor to max size
        padding_size = (0, 0, 0, max_size.item() - local_size.item())
        padded_tensor = F.pad(tensor, padding_size, "constant", 0)

        # Gather all tensors
        gathered_tensors = [torch.zeros_like(padded_tensor) for _ in range(get_world_size())]
        dist.all_gather(gathered_tensors, padded_tensor)

        ctx.rank = get_rank()
        ctx.local_size = local_size.item()
        ctx.all_sizes = all_sizes
        ctx.max_size = max_size.item()
        
        # Remove padding for forward output
        output = torch.cat([t[:size.item()] for t, size in zip(gathered_tensors, all_sizes)], dim=0)
        return output

    @staticmethod
    def backward(ctx, grad_output):
        # Split the grad_output into chunks based on all_sizes
        dist.all_reduce(grad_output)
        chunks = grad_output.split([size.item() for size in ctx.all_sizes], dim=0)

        # Only return the portion of the gradient that corresponds to the original local tensor size
        return chunks[ctx.rank], None