...
import ocpmodels
from ocpmodels.modules.loss import AtomwiseL2Loss, L2MAELoss, RankSimLoss, ConRLoss, QuantileLoss, BMCLoss
...
def build_config(args, args_override):
    ...
    # newly add
    config['early-stop'] = args.early_stop
    config['linear-probe'] = args.linear_probe
...
def new_trainer_context(*, config: Dict[str, Any], args: Namespace):
    ...
    try:
        ...
        trainer_cls = registry.get_trainer_class(trainer_name) # ocp: ocpTrainer
        ...
        trainer = trainer_cls(
            ...
            early_stop=config.get("early-stop", 30),
            linear_probe=config.get("linear-probe", False),
            name=task_name,
        )
...
def get_loss_module(loss_name, **kwargs):
    if loss_name in ["l1", "mae"]:
        ...
        loss_fn = QuantileLoss(tau=kwargs.get("tau", 0.5))
    elif loss_name == 'ranksim':
        loss_fn = RankSimLoss(
            lambda_val=kwargs.get("lambda_val", 2)
        )
    elif loss_name == 'conr':
        loss_fn = ConRLoss(threshold=kwargs.get("threshold", 0.1),
                           temperature=kwargs.get("temperature", 0.07),
                           eta=kwargs.get("eta", 0.01),
                           global_con=kwargs.get("global_con", False))
    elif loss_name == 'bmc':
        loss_fn = BMCLoss(init_noise_sigma=kwargs.get("init_noise_sigma", 1.0),
                          global_con=kwargs.get("global_con", False))
    else:
