...
class EquiformerV2_OC20(BaseModel):
    ...
    def get_mol_rep(self, data, merge: str = 'avg'):
        x = self.get_atom_reps(data) # (num_atoms, num_types, hidden)
        atom_embedding = x.embedding.narrow(1, 0, 1) # (num_atoms, 1, hidden), extract type-0 vector, which is SE(3)-invariant features
        ...
        mol_embedding.index_add_(0, data.batch, atom_embedding.view(-1, self.sphere_channels)) # sum! switch to mean!
        if merge == 'avg':
            mol_embedding /= (data.natoms.unsqueeze(-1)) # we use mean pooling here
        else:
            raise ValueError('Only support avarage feature pooling!')
        return mol_embedding

    @conditional_grad(torch.enable_grad())
    def forward(self, data):
        x = self.get_atom_reps(data)
        ...
        energy.index_add_(0, data.batch, node_energy.view(-1)) # (b, )
...
@registry.register_model("equiformer_v2-direct")
class EquiformerV2Direct(EquiformerV2_OC20):
    def __init__(
        self,
        num_atoms: int,  # not used
        bond_feat_dim: int,  # not used
        ...
        load_energy_lin_ref: Optional[bool] = False,
        task_name: str = None,
        atom_noise_scale: float = None,
        cell_noise_scale: float = None,
    ):
        super().__init__(        
            num_atoms,  # not used
        ...
        self.cell_noise_scale = cell_noise_scale
        self.prop_heads = nn.ModuleDict({
            self.task_name: nn.Sequential(
                nn.Linear(
                    self.sphere_channels, self.sphere_channels, bias=True
                ),
                nn.SiLU(),
                nn.Linear(
                    self.sphere_channels, 1, bias=True
                ),
            )
        })
    ...
    def forward(self, data, return_embed: bool=True):
        if self.atom_noise_scale is not None and self.training:
            atom_noise = self.atom_noise_scale * \
                torch.randn_like(data.pos, dtype=data.pos.dtype, device=data.pos.device)
            data.pos += atom_noise
        
        if self.cell_noise_scale is not None and self.training:
            cell_noise = self.cell_noise_scale * \
                torch.randn_like(data.cell, dtype=data.cell.dtype, device=data.cell.device)
            data.cell += cell_noise

        mol_embedding = self.get_mol_rep(data)
...
@registry.register_model("equiformer_v2-mex")
class EquiformerV2MEX(EquiformerV2_OC20):
    def __init__(
        self,
        num_atoms: int,  # not used
        bond_feat_dim: int,  # not used
        ...
        self.atom_noise_scale = atom_noise_scale
        self.cell_noise_scale = cell_noise_scale
        if self.atom_noise_scale is not None or self.cell_noise_scale is not None:
            logging.info(f'atom noise: {self.atom_noise_scale}, cell noise: {self.cell_noise_scale}')
        ...
        self.fc1_xy = nn.Sequential(
            nn.Linear(self.sphere_channels * 2, self.sphere_channels),
            nn.SiLU(),
        )
        ...
        self.fc4_xy = nn.Linear(self.sphere_channels, 1)
    ...
    def get_energy(self, x_feature, y, ret_rep=False):
        ...
        # fused_energy_score = F.cosine_similarity(x_feat_post , y_feat_post) # (batch_size*num_samples, )
        # fused_energy_score = fused_energy_score.view(batch_size, num_samples) # (batch_size, num_samples)

        if ret_rep:
            return fused_energy_score, x_feature, y_feature
        else:
            return fused_energy_score

    def get_mol_rep(self, data):
        '''
        override get_mol_rep to support add noise
        '''
        if self.atom_noise_scale is not None and self.training:
            atom_noise = self.atom_noise_scale * \
                torch.randn_like(data.pos, dtype=data.pos.dtype, device=data.pos.device)
        ...
        mol_repr = super().get_mol_rep(data)
        mol_repr = self.fc_x(mol_repr)
