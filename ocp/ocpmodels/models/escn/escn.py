...
@registry.register_model("escn-mine")
class eSCNmine(eSCN):
    """Equivariant Spherical Channel Network
    Paper: Reducing SO(3) Convolutions to SO(2) for Efficient Equivariant GNNs
    ...
    def __init__(
        ...
        super().__init__(num_atoms,  # not used
    ...
    def forward(self, data,return_embed: bool=True):
        ...
        # Sample the spherical channels (node embeddings) at evenly distributed points on the sphere.
        # These values are fed into the output blocks.
        atom_embedding = x.embedding.narrow(1, 0, 1)  # (num_atoms, 1, hidden)
        ...
        mol_embedding.index_add_(0, data.batch, atom_embedding.view(-1, self.sphere_channels))  # (b, hidden)
...
@registry.register_model("escn-multitask")
class eSCNmulti(eSCN):
    def __init__(
        self,
        num_atoms: int,  # not used
        bond_feat_dim: int,  # not used
    ...
    @conditional_grad(torch.enable_grad())
    def forward(self, data,name_bz: List[Tuple[str, torch.Tensor]]):
        device = data.pos.device
        self.batch_size = len(data.natoms)
        self.dtype = data.pos.dtype
        ...
        bzs = []
        for (name, bz) in name_bz:
            assert name in self.task_names
            bzs.append(bz)
        assert torch.sum(torch.tensor(bzs)) == data.batch.max() + 1
        atom_embedding = x.embedding.narrow(1, 0, 1)  # (num_atoms, 1, hidden)
        mol_embedding = torch.zeros(
            len(data.natoms),
            self.sphere_channels,
        ...
        #print(bzs)
        mol_embedding /= (data.natoms.unsqueeze(-1))
        mol_embedding = torch.split(mol_embedding, bzs, dim=0)
        res = {}
        for i, emb in enumerate(mol_embedding):
            res[name_bz[i][0]] = self.prop_heads[name_bz[i][0]](emb)  # (b, 1)
            res[f'{name_bz[i][0]}_embeddings'] = emb

        return res
    
    @torch.no_grad()
    def repr_to_label(self, embed: torch.Tensor, prop_name: str) -> torch.Tensor:
        '''
        embed: (B, h)
        '''
        assert embed.dim() == 2, f'bad embed shape: {embed.shape}'
        assert prop_name in self.prop_heads, f'{prop_name} not in self.prop_heads: {self.prop_heads.keys()}'

        pred = self.prop_heads[prop_name](embed) # (b, 1)
        return pred
...
@registry.register_model("escn-ebm")
class eSCNEBM(eSCN):
    """Equivariant Spherical Channel Network
    Paper: Reducing SO(3) Convolutions to SO(2) for Efficient Equivariant GNNs
