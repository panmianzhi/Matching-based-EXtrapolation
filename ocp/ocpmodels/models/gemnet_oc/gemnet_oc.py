...
class GemNetOC(BaseModel):
    ...
    def get_atom_reps(self, data):
        ...
        # return h
        # Global output block for final predictions
        return self.out_mlp_E(torch.cat(xs_E, dim=-1))  # (num_atoms, dim)
...
@registry.register_model("gemnet_oc-mine")
class GemNetOCMine(GemNetOC):
    def __init__(
        self,
        num_atoms: Optional[int],
        bond_feat_dim: int,
        ...
        **kwargs,  # backwards compatibility with deprecated arguments
        ...
        self.prop_heads = nn.ModuleDict({
            self.task_name: nn.Sequential(
                nn.Linear(
                    self.emb_size_atom, self.emb_size_atom, bias=True
                ),
                nn.SiLU(),
                nn.Linear(
                    self.emb_size_atom, 1, bias=True
                ),
            )
        })
    ...
    @conditional_grad(torch.enable_grad())
    def forward(self, data,return_embed: bool=True):
        x_E = self.get_atom_reps(data)
        atom_embedding = x_E  # (num_atoms, 1, hidden)
        ...
        assert atom_embedding.shape[-1] == mol_embedding.shape[-1], \
            f'atom_embedding: {atom_embedding.shape}, mol_embedding: {mol_embedding.shape}'
        mol_embedding.index_add_(0, data.batch, atom_embedding.view(-1, self.emb_size_atom))  # (b, hidden)
        mol_embedding /= (data.natoms.unsqueeze(-1))
        res = {
            self.task_name: self.prop_heads[self.task_name](mol_embedding)
        }  # (b, 1)
        if return_embed:
            res["embeddings"] = mol_embedding
        return res

@registry.register_model("gemnet_oc-foma")
class GemNetOCfoma(GemNetOC):
    def __init__(
        self,
        num_atoms: Optional[int],
        bond_feat_dim: int,
    ...
    def calculate_k(self,sigma_values, rho):
        # Step 1: Calculate the total sum of sigma values
        total_sum = sum(sigma_values)
        # Step 2: Initialize variables for cumulative sum and k
        cumulative_sum = 0
        k = 0
        # Step 3: Iterate through the sigma values
        for i, sigma in enumerate(sigma_values):
            cumulative_sum += sigma
            k = i + 1  # since index i is 0-based, k should be i+1
            # Step 4: Check if the cumulative sum ratio exceeds rho
            if cumulative_sum / total_sum >= rho:
                break
        return k

    @conditional_grad(torch.enable_grad())
    def forward(self, data,is_train: bool=True,return_embed: bool=True):
        ...
        if is_train==True:
            try:
                lam = torch.distributions.beta.Beta(torch.tensor([0.5]), torch.tensor([0.5])).sample().to(
                    atom_embedding.device)
                A = torch.cat((mol_embedding, y), dim=1)
                U, s, Vt = torch.linalg.svd(A, full_matrices=False)
                k = self.calculate_k(s, 0.9)
                lam_repeat = lam.repeat(s.shape[-1] - k)
                lam_ones = torch.ones(k).to(atom_embedding.device)
                s = s * torch.cat((lam_ones, lam_repeat))
                A = U @ torch.diag(s) @ Vt
                mol_embedding_foma, y = A[:, :mol_embedding.shape[1]], A[:, mol_embedding.shape[1]:]
                res = {
                    self.task_name: self.prop_heads[self.task_name](mol_embedding_foma)
                }  # (b, 1)
                if return_embed:
                    res["embeddings"] = mol_embedding_foma
                return res, y
            except:
                res = {
                    self.task_name: self.prop_heads[self.task_name](mol_embedding)
...
@registry.register_model("gemnet_oc-mixup")
class GemNetOCMixup(GemNetOC):
    def __init__(
        self,
        num_atoms: Optional[int],
        bond_feat_dim: int,
        ...
        **kwargs,  # backwards compatibility with deprecated arguments
        ...
        self.out_forces = None
    @conditional_grad(torch.enable_grad())
    def forward(self, data,lmabd,return_embed: bool=True):
        x_E = self.get_atom_reps(data[0])
        atom_embedding0 = x_E  # (num_atoms, 1, hidden)
        mol_embedding0 = torch.zeros(
            len(data[0].natoms),
            self.emb_size_atom,
            device=atom_embedding0.device,
            dtype=atom_embedding0.dtype,
        )  # (b, hidden)
        assert atom_embedding0.shape[-1] == mol_embedding0.shape[-1], \
            f'atom_embedding: {atom_embedding0.shape}, mol_embedding: {mol_embedding0.shape}'
        mol_embedding0.index_add_(0, data[0].batch, atom_embedding0.view(-1, self.emb_size_atom))  # (b, hidden)
        mol_embedding0 /= (data[0].natoms.unsqueeze(-1))
        if len(data)==1:
            res = {
                self.task_name: self.prop_heads[self.task_name](mol_embedding0)
            }  # (b, 1)
            if return_embed:
                res["embeddings"] = mol_embedding0
            return res
        elif len(data)==2:
            x_E = self.get_atom_reps(data[1])
            atom_embedding1 = x_E  # (num_atoms, 1, hidden)
            mol_embedding1 = torch.zeros(
                len(data[1].natoms),
                self.emb_size_atom,
                device=atom_embedding1.device,
                dtype=atom_embedding1.dtype,
            )  # (b, hidden)
            assert atom_embedding1.shape[-1] == mol_embedding1.shape[-1], \
                f'atom_embedding: {atom_embedding1.shape}, mol_embedding: {mol_embedding1.shape}'
            mol_embedding1.index_add_(0, data[1].batch, atom_embedding1.view(-1, self.emb_size_atom))  # (b, hidden)
            mol_embedding1 /= (data[1].natoms.unsqueeze(-1))
            mol_embedding_mix = mol_embedding0 * lmabd + mol_embedding1 * (1 - lmabd)
            res = {
                self.task_name: self.prop_heads[self.task_name](mol_embedding_mix)
            }  # (b, 1)
            if return_embed:
                res["embeddings"] = mol_embedding_mix
            return res

@registry.register_model("gemnet_oc-ebm")
class GemNetOCebm(GemNetOC):
    def __init__(
        self,
        num_atoms: Optional[int],
        bond_feat_dim: int,
        ...
        **kwargs,  # backwards compatibility with deprecated arguments
        ...
        self.task_name = task_name
        self.emb_size_atom = emb_size_atom
        self.energy_block = None  # dismiss the origin energy block
        self.fc_y = nn.Sequential(
            nn.Linear(1, self.emb_size_atom),
            nn.SiLU(),
        )
        # x proj
        ...
        # xy encoder. fuse x_feat and y_feat to obtain the fused energy model
        # self.fc1_xy = nn.Sequential(
        #     nn.Linear(self.emb_size_atom * 2, self.emb_size_atom),
        #     nn.SiLU(),
        # )
        # self.fc2_xy = nn.Sequential(
        #     nn.Linear(self.emb_size_atom, self.emb_size_atom),
        #     nn.SiLU(),
        # )
        # self.fc3_xy = nn.Sequential(
        #     nn.Linear(self.emb_size_atom, self.emb_size_atom),
        #     nn.SiLU(),
        # )
        # self.fc4_xy = nn.Linear(self.emb_size_atom, 1)

        # post X-Enc and Y-Enc. Perform NCE in another space
        self.post_mlp_x = nn.Sequential(
            nn.Linear(self.emb_size_atom, self.emb_size_atom),
            nn.SiLU(),
            nn.Linear(self.emb_size_atom, self.emb_size_atom),
        )
        self.post_mlp_y = nn.Sequential(
            nn.Linear(self.emb_size_atom, self.emb_size_atom),
            nn.SiLU(),
            nn.Linear(self.emb_size_atom, self.emb_size_atom),
        )

    def get_mol_rep(self, data, merge: str = 'avg'):
        x = self.get_atom_reps(data) # (num_atoms, 2l+1, hidden)
        atom_embedding = x # (num_atoms, 1, hidden)
        ...
        assert atom_embedding.shape[-1] == mol_embedding.shape[-1], \
            f'atom_embedding: {atom_embedding.shape}, mol_embedding: {mol_embedding.shape}'
        mol_embedding.index_add_(0, data.batch, atom_embedding.view(-1, self.emb_size_atom)) # sum! switch to mean!
        if merge == 'avg':
            mol_embedding /= (data.natoms.unsqueeze(-1)) # we use mean pooling here
        else:
            raise NotImplementedError
        return mol_embedding

    def get_energy(self, x_feature, y, ret_rep=False):
        '''
        x_feature: (B, D)
        y: (B,) or (B, 1) or (B, num_sample)
        '''
        if y.dim() == 1:
            y = y.view(-1, 1)
        ...
        # Replicate
        x_feature = x_feature.view(batch_size, 1, -1).expand(-1, num_samples,
        ...
        x_feature = self.fc_x(x_feature)

        y = y.reshape(batch_size * num_samples, -1)  # (shape: (batch_size*num_samples, 1))
        y_feature = self.get_y_rep(y)

        ####### distance to obtain energy score #####
        # cosine_similarity = F.cosine_similarity(x_feature, y_feature) # (batch_size*num_samples, )
        # cos_energy_score = cosine_similarity.view(batch_size, num_samples)

        # fuse to obtain energy score
        # xy_feature = torch.cat([x_feature, y_feature], 1)  # (batch_size*num_samples, 2*hidden_dim)
        # xy_feature = self.fc1_xy(xy_feature)
        # xy_feature = self.fc2_xy(xy_feature) + xy_feature
        # xy_feature = self.fc3_xy(xy_feature) + xy_feature
        # fused_energy_score = self.fc4_xy(xy_feature)  # (batch_size*num_samples, 1)
        # fused_energy_score = fused_energy_score.view(batch_size, num_samples)  # (batch_size, num_samples)

        # post feat energy
        x_feat_post = self.post_mlp_x(x_feature)
        y_feat_post = self.post_mlp_y(y_feature)
        post_cos_energy = F.cosine_similarity(x_feat_post , y_feat_post) # (batch_size*num_samples, )
        post_cos_energy = post_cos_energy.view(batch_size, num_samples) # (batch_size, num_samples)

        if ret_rep:
            return post_cos_energy, x_feature, y_feature
        else:
            return post_cos_energy

    @conditional_grad(torch.enable_grad())
    def forward(self, data):
        mol_embedding = self.get_mol_rep(data)  # (B, dim)
        return self.get_energy(mol_embedding, data[self.task_name])

    def get_y_rep(self, y: torch.tensor):
        assert y.dim() == 2 and y.shape[-1] == 1
        y_feature = self.fc_y(y)
