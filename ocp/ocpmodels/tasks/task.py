...
#added by panmz
@registry.register_task("analyse")
class AnalyseTask(BaseTask):
    def run(self) -> None:
        assert self.config["checkpoint"]
        
        saved_path = os.path.join(
            *self.config["checkpoint"].split("checkpoints")[0].split(os.sep), 
            "results",
            self.config["checkpoint"].split("checkpoints")[1].split(os.sep)[1]
        )
        self.trainer.analyse(saved_path)

@registry.register_task("test_validation")
class PredictValTask(BaseTask):
    def run(self) -> None:
        assert (
            self.trainer.val_loader is not None
        ), "Test dataset is required for making predictions"
        ...
        self.trainer.test_validation()

@registry.register_task("inference")
class InferTask(BaseTask):
    def run(self) -> None:
        ...
        self.trainer.encode_x(saved_path)

@registry.register_task("sae_infer")
class SAEInferTask(BaseTask):
    def run(self) -> None:
        assert self.config["checkpoint"]
        ...
        self.trainer.SAE_inference(saved_path)