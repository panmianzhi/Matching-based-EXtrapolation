...
mp_dielectric_small_param=(64 0.001 0)
...
cfg_root_dir="configs/extrapolation_benchmark_eqv2_lds"
cur_algo="lds"
m_port=721
d_port=821
...
for cfg_n in "${cfg_names[@]}"; do
    ...
    for seed in {1..2}; do
        ...
        --seed $seed \
        --run-dir record_shared_etp_v3/${cur_algo}/eqv2-scr_lr=${lr}_wd=${wd}_bs=${NUM_GPUS}-${bs}/${cfg_n} \
        --amp \
        --num-gpus $NUM_GPUS \
        --distributed \
